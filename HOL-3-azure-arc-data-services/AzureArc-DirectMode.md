# HOL 03: Exercise 7: Deploying Azure Arc Data Controller with direct connectivity mode 
...
1. Run the below command to upgrade the `az` extension.
   ...
   > It may take some time. Wait till the installation process is completed.
...
1. Run the below commands to install the required Azure CLI extensions.
   ...
   ```   
    ...
    ![](media/install-extensions.png "Lab Environment")
     ...
     >**Note:** If you get any warnings, please ignore them. They are not errors.    
...
1. Run the below commands to register the required resource providers if they are not already registered.
...
Custom Locations provide administrators a way to deploy Azure Arc data services and other Azure Arc-enabled services to their locations, similar to Azure Locations.
...
1. Run the below command to deploy the extension of Azure Arc-enabled Data Services on the Azure Arc Kubernetes cluster.
   ...
   > **Note:** The above command will take up to 5 minutes to complete the creation of the azdata extension; please wait for it to complete before proceeding to the next task.
...
1. Now run the below command to create a custom location by referencing the Azure Arc-enabled Kubernetes cluster ID and the extension ID.
   ...
   > **Note:** It can take up to 2 minutes to complete the creation of a custom location. The output should be as shown below:    
...
1. Now, on the ```Create Azure Arc data controller``` blade, select **Azure Arc-enabled Kubernetes (Direct connectivity mode) (1)**. and click on **Next: Data Controller details (2)**.
...
1. On the **Data controller details** blade, enter the following details:
   ...
   * Custom location: Select the available custom location from the dropdown **(4)**.
...
1. On the Review + Create blade, you can check all the given details and click on the **Create** button to start the Azure Arc data controller deployment.
...
## Task 5: Monitor the creation of the Azure Arc data controller on the cluster.
...
1. Once the data controller state is changed to ready, proceed to the next steps. Please note that the data controller deployment can take 5-10 minutes to change it to ready.
